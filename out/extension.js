...
class SourceStore {
    ...
    getAll() {
        return this.state.get(SourceStore.KEY) ?? [];
    }
    async add(source) {
        const all = this.getAll();
        if (all.some((s) => s.path === source.path && s.type === source.type))
            return false;
        await this.state.update(SourceStore.KEY, [...all, source]);
        return true;
    }
    async remove(sourcePath) {
        await this.state.update(SourceStore.KEY, this.getAll().filter((s) => s.path !== sourcePath));
    }
}
SourceStore.KEY = "scriptRunner.sources";
// ---------------------------------------------------------------------------
...
function detectPM(pkgDir) {
    ...
    if (fs.existsSync(path.join(pkgDir, "pnpm-lock.yaml")))
        return "pnpm";
    if (fs.existsSync(path.join(pkgDir, "bun.lockb")))
        return "bun";
    if (fs.existsSync(path.join(pkgDir, "yarn.lock")))
        return "yarn";
...
// ---------------------------------------------------------------------------
function parseMakeTargets(filePath) {
    try {
...
class ScriptItem extends vscode.TreeItem {
    constructor(script, extensionUri, overrides, labels) {
        const collapsible = script.kind === "packageGroup" ||
        ...
        switch (script.kind) {
            ...
            case "npmScript":
                ...
                if (!customLabel)
                    this.description = effectiveCommand;
...
class ScriptProvider {
    constructor(extensionUri, sources, overrides, labels) {
        this.extensionUri = extensionUri;
        ...
        for (const src of all.filter((s) => s.type === "directory")) {
            for (const name of ["Makefile", "makefile", "GNUmakefile"]) {
                ...
                if (!fs.existsSync(mkPath))
                    continue;
                if (!parseMakeTargets(mkPath).length)
                    break;
    ...
    npmScripts(group) {
        try {
            ...
            return Object.entries(pkg.scripts ?? {}).map(([name, cmd]) => this.item({
                kind: "npmScript",
                label: name,
    ...
    // -- Makefile ------------------------------------------------------------
    makeTargets(group) {
        return parseMakeTargets(group.filePath).map((target) => this.item({
            kind: "makeTarget",
            label: target,
    ...
    async fileCategoryHeaders(all) {
        ...
        for (const type of FILE_SCRIPT_TYPES) {
            if (!type.extensions.length)
                continue; // makefile handled separately
            let hasAny = false;
            // Check directories
    ...
    fileCategoryChildren(cat) {
        ...
        for (const src of all.filter((s) => s.type === "file")) {
            if (seen.has(src.path))
                continue;
            if (!type.extensions.includes(path.extname(src.path)))
                continue;
            seen.add(src.path);
    ...
    fileScriptItem(absPath, cwd, type, filename) {
        return this.item({
            kind: "fileScript",
...
function activate(context) {
    ...
    updateMessage();
    context.subscriptions.push(treeView, vscode.commands.registerCommand("scriptRunner.refresh", () => {
        provider.refresh();
        updateMessage();
    }), vscode.commands.registerCommand("scriptRunner.run", (script, effectiveCommand) => runInTerminal(script, effectiveCommand)), vscode.commands.registerCommand("scriptRunner.setOverride", async (item) => {
        const { script } = item;
        if (!script.id)
            return;
        ...
        provider.refresh();
    }), vscode.commands.registerCommand("scriptRunner.clearOverride", async (item) => {
        if (!item.script.id)
            return;
        await overrides.clear(item.script.id);
        provider.refresh();
    }), vscode.commands.registerCommand("scriptRunner.setLabel", async (item) => {
        const { script } = item;
        if (!script.id)
            return;
        ...
        provider.refresh();
    }), vscode.commands.registerCommand("scriptRunner.clearLabel", async (item) => {
        if (!item.script.id)
            return;
        await labels.clear(item.script.id);
        provider.refresh();
    }), vscode.commands.registerCommand("scriptRunner.removeSource", async (item) => {
        ...
        provider.refresh();
        updateMessage();
    }), vscode.commands.registerCommand("scriptRunner.manageSources", async () => {
...
// Terminal runner
// ---------------------------------------------------------------------------
function runInTerminal(script, command) {
...
// Add sources
// ---------------------------------------------------------------------------
async function addPackageJson(sources, provider, updateMessage) {
    ...
    for (const uri of picked) {
        ...
        if (await sources.add({ type: "packageJson", path: absPath }))
            added++;
    }
    if (added > 0) {
        provider.refresh();
...
async function addDirectory(sources, provider, updateMessage) {
    const wsRoot = vscode.workspace.workspaceFolders?.[0].uri;
    const picked = await vscode.window.showOpenDialog({
    ...
    let added = 0;
    for (const uri of picked) {
        if (await sources.add({ type: "directory", path: uri.fsPath }))
            added++;
    }
    if (added > 0) {
        provider.refresh();
...
async function addFile(sources, provider, updateMessage) {
    const wsRoot = vscode.workspace.workspaceFolders?.[0].uri;
    ...
    for (const uri of picked) {
        ...
        if (await sources.add({ type: "file", path: uri.fsPath }))
            added++;
    }
    if (added > 0) {
        provider.refresh();
